...
class MultiValueDictFieldTranslatorTest {
    ...
    void multiValueDictTest() {
        ...
        testObject.getSingleStatusList().add("1");
        testObject.getSingleStatusList().add("2");
        testObject.getSingleStatusList().add("3");
        ...
        // 验证空字符串情况
        assertEquals("", dictResult.getString("emptyStatus"));
        ...
        //集合测试
        assertEquals(testObject.getSingleStatusList().size(), dictResult.getJSONArray("singleStatusList").size());
        assertTrue(dictResult.getJSONArray("singleStatusList").contains("未开始"));
        assertTrue(dictResult.getJSONArray("singleStatusList").contains("进行中"));
        assertTrue(dictResult.getJSONArray("singleStatusList").contains("已完成"));
    ...
    public static class MultiValueTestObject implements Dict {
        ...
        // 集合测试
        @DictMapping(dictCode = "taskStatus")
        private List<String> singleStatusList = new ArrayList<>();
    }
}